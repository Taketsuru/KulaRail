...
public class TeleporterManager implements Listener, SignedBlockListener {
	...
	SignedBlock entry = signedBlockManager.getSignedBlockAt(blockCollidedWith.getLocation());
	...
	for (SignedBlock block : channel) {
	    ...
	    double distanceSquared = block.getLocation().distanceSquared(entry.getLocation());
	...
	for (int dir = 0; dir < faces.length; ++dir) {
	    ...
	    case EAST:
	    case WEST:
		attached = face.equals(BlockFace.EAST) || face.equals(BlockFace.WEST);
		break;

	    case NORTH:
	    case SOUTH:
		attached = face.equals(BlockFace.SOUTH) || face.equals(BlockFace.NORTH);
		break;
		...
		vehicle.eject();
		Location newLocation = exitLocation.clone().add(face.getModX() + 0.5, face.getModY() + 0.35, face.getModZ() + 0.5);
	...
	return player.hasPermission(isEntryBlock(block) ? "kularail.entry" : "kularail.exit"); 
    }
    ...
    HashMap<String, Vector<SignedBlock>> channels = new HashMap<String, Vector<SignedBlock>>();
}
