...
public class SignedBlockManager implements Listener {
	...
	org.bukkit.material.Sign data = new org.bukkit.material.Sign(signBlock.getType(), signBlock.getData());
	...
	if (block == null) {
		...
		blockLocation = getAttachedBlock(event.getBlock()).getLocation();
		block = getSignedBlockAt(blockLocation);
		if (block != null && ! block.getSignBlock().equals(event.getBlock())) {
		    block = null;
		}
	    }
	}
	...
	for (Iterator<Chunk> iter = countInEachChunk.keySet().iterator(); iter.hasNext(); ) {
	...
	SignedBlock block = new SignedBlock(selfBlock.getLocation(), signBlock.getLocation(), event.getLines());
